...
export function cumProfile(hourlyRows, compDays) {
  ...
  for (const r of hourlyRows) if (set.has(r.datum)) acc[opIdx(+r.hodina)] += +r.joblines || 0;
  const s = acc.reduce((a, b) => a + b, 0) || 1;
  let run = 0;
  return acc.map((v) => (run += v / s)); // index = prevádzkové poradie (0 = 06:00)
}
...
// Backlog = koľko JBL sa v postihnutých hodinách NESPRACOVALO oproti čistému očakávaniu.
export function backlogForDay(datum, hodiny, hourlyRows, cleanTotal, prof) {
  const pr = prof[String(dow(datum) >= 5)];
  const hrs = new Map();
  for (const r of hourlyRows) if (r.datum === datum) hrs.set(+r.hodina, (hrs.get(+r.hodina) || 0) + (+r.joblines || 0));
  let backlog = 0;
  const detail = [];
  for (const h of hodiny) {
    const exp = cleanTotal * (pr[h] || 0);
    const act = hrs.size ? (hrs.get(h) || 0) : 0;
    const b = Math.max(0, exp - act);
    backlog += b;
    detail.push({ hodina: h, ocak: exp, skut: act, backlog: b });
  }
  return { backlog, detail, maHodinoveData: hrs.size > 0 };
}
