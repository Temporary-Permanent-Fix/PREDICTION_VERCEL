...
export function fitModel(daily, vynimkyDates, udalosti) {
  ...
  return { dowF, domF, levelNow, slope, damp: 0.977, lastDate, trainDays: df.length, residStd, expectedHist, defaultKoef };
}
...
export function predictDay(datum, model, udalosti) {
  ...
  return level * model.dowF[dow(datum)] * model.domF[dom(datum)] * eventMult(datum, udalosti);
}
